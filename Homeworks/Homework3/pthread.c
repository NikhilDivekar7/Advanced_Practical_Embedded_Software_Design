...
struct node * data_process(struct node * head)
{
	...
	mainptr = fopen("Output.txt", "a");
	pthread_mutex_lock(&pmutex);
	logger(mainptr, "Character occured thrice", "First thread", a, getppid(), getpid(), syscall(SYS_gettid));
 	pthread_mutex_unlock(&pmutex);
	arr[i] = a;
	i++;
	}
...
int main()
{
	...
	child_thread1 -> file_name = "Valentinesday.txt";
	child_thread1 -> output = "Output.txt";
	child_thread1 -> child_number = 1;

	child_thread2 -> file_name = "Valentinesday.txt";
